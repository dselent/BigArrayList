...
public class CreateSortTestScripts
{

	public CreateSortTestScripts()
	{
		...
		for(int i=0; i<numberOfElements.length; i++)
		{
			...
			{
				String line = "java -Xmx10000M SortTestRandomArrayList " + numberOfElements[i] + " " + (l+1) + " >> SortTestRandomOutput.txt";
				line = line.concat("\n");
				line = line.concat("timeout /t 2");

				try
				{
					FileWriter file = new FileWriter("SortTests.bat", true);
					file.write(line);
		...
		for(int i=0; i<numberOfElements.length; i++)
		{
			for(int j=0; j<tableSizes.length; j++)
			{
				for(int k=0; k<cacheBlocks.length; k++)
				{
					...
					{
						if(numberOfElements[i] / tableSizes[j] <= 10000)
						{
							String line = "java -Xmx10000M SortTestRandomBigArrayList " + numberOfElements[i] + " " + tableSizes[j] + " " + cacheBlocks[k] + " " + (l+1) + " >> SortTestRandomBigOutput.txt";
							line = line.concat("\n");
							line = line.concat("timeout /t 2");
